  ...
  ds_ls <- lapply(response, function(dev) {
    lapply(dev, function(x) {
      tryCatch({
        ...
        return(NULL)#' Obtain an Access Token from a Keycloak Server Using Resource Owner Password Credentials
#'
#' Requests an OAuth2 access token from a Keycloak authentication server using the Resource Owner Password Credentials Grant (password grant).
#'
#' @param url Character. The token endpoint URL of the Keycloak server (typically ends with \code{/protocol/openid-connect/token}).
...
.locate_sta_datastreams <- function(url, token = NULL, var, lon, lat,  radius = 0, from, to,...){
      ...
      dplyr::bind_rows(
        lapply(ds_ls[[i]], function(ds) {
          ...
          tibble::tibble(
            ...
            tidyr::separate(
              phenomenonTime, into = c("start_date","end_date"), sep = "/"
            ) %>%
            dplyr::mutate(
              dplyr::across(start_date:end_date, ~ as.Date(.x))
            )
        })
      )
  }
  ...
  out <- out %>%
    dplyr::filter(ObservedProperty.name %in% var)
  if (nrow(out) == 0) {
  ...
  out <- out %>%
    dplyr::mutate(
      is_contained = start_date >= as.Date(from) & end_date <= as.Date(to)
    ) %>%
    dplyr::filter(is_contained)
  
  if (nrow(out) == 0) {
    return("Measured data does not encompass the requested timeframe.")
  } else {
    return(out %>% dplyr::select(-is_contained)) # Remove the temporary column
  }
}
